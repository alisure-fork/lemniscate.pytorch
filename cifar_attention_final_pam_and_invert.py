...
class NearestAttention(nn.Module):
    ...
    def __init__(self, n_sample, k_nearest, pam_size, final_size, t=0.1, momentum=0.5):
    ...
    def forward(self, pam_features, final_features, indexes):
        # k_nearest_indexes
        k_score, k_nearest_pam_features, k_invert_pam_features = self._get_k_nearest(
            pam_features.data, final_features.data, self.k_nearest)

        # update features
        self.update_features(pam_features, final_features, indexes)
        ...
        return k_score, k_nearest_pam_features, k_invert_pam_features, linear_average_out

    def update_features(self, pam_features, final_features, indexes):
        # update pam
        ...
        self.pam_memory.index_copy_(0, indexes, updated_weight)
    ...
    def init_features(self, pam_features, final_features, indexes):
        # init pam
    ...
    def _get_k_nearest(self, pam_features, final_features, k_nearest):
        # dist
        ...
        k_invert_pam_features = self.pam_memory.index_select(
            0, k_invert_indexes.data.view(-1)).resize_(pam_features.size(0), self.k_nearest, self.pam_size)

        return k_nearest_score, k_nearest_pam_features, k_invert_pam_features
...
class PAMModule(nn.Module):
    ...
    def forward(self, k_nearest_score, pam_features, k_nearest_pam_features):
        ...
        final_out = k_nearest_pam_features + cat_out
        out = torch.bmm(k_nearest_score.view(batch_size, 1, -1), final_out).squeeze()
        return out
...
class AttentionLoss(nn.Module):
    ...
    def forward(self, pam_features, pam_features_2, pam_features_invert, linear_average_out, indexes):
        loss_1 = self.criterion(pam_features, pam_features_2) * self.k_nearest
        loss_2 = self.criterion_no(linear_average_out, indexes)
        loss_3 = self.criterion(pam_features, pam_features_invert) * self.k_nearest
        # return loss_1 + loss_2 + loss_3, loss_1, loss_2, loss_3
        # return loss_2 + loss_3, loss_1, loss_2, loss_3
        return loss_2, loss_1, loss_2, loss_3
...
class AttentionRunner(object):
    ...
    def __init__(self, k_nearest=11, momentum=0.5, learning_rate=0.03, resume=False,
        ...
        self.net, self.nearest_attention, self.pam_module, self.criterion = self._build_model()
    ...
    def _build_model(self):
        ...
        return net, nearest_attention, pam_module, criterion
    ...
    def _train_one_epoch(self, epoch, max_epoch):
        ...
        for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
            ...
            k_nearest_score, k_near_pam_features, k_invert_pam_features, linear_average_out = self.nearest_attention(
                pam_features, feature_final, indexes)
            pam_features_2 = self.pam_module(k_nearest_score, pam_features, k_near_pam_features)
            pam_features_invert = -k_invert_pam_features.mean(dim=1)

            loss, loss_1, loss_2, loss_3 = self.criterion(pam_features, pam_features_2, pam_features_invert,
...
if __name__ == '__main__':
    ...
    pre_train = None
    # pre_train = "./checkpoint/attention_l2norm/ckpt.t7"
    runner = AttentionRunner(k_nearest=_k_nearest, resume=False, pre_train=pre_train,
                             checkpoint_path="./checkpoint/attention_invert/ckpt.t7")
