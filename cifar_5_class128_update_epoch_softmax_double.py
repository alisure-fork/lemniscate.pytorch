...
class AttentionResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, low_dim=128):
        ...
        self.linear_128 = nn.Linear(512 * block.expansion, low_dim)
    ...
    def forward(self, x):
        ...
        out = self.linear_128(out)
        out_search = self.l2norm(out)
        out_logits = torch.cat((out_search, -out_search), dim=-1)
        out_softmax = self.softmax(out_logits)
        return out_search, out_logits, out_softmax
    ...
    def knn(epoch, net, produce_class, train_loader, test_loader, k, t, recompute_memory=0, loader_n=1):
        ...
        if recompute_memory:
            ...
            for batch_idx, (inputs, _, indexes) in enumerate(temp_loader):
                out_search, out_logits, out_softmax = net(inputs)
                batch_size = inputs.size(0)
                out_memory[:, batch_idx * batch_size:batch_idx * batch_size + batch_size] = out_search.data.t()
                pass
        ...
        with torch.no_grad():
            ...
            for loader in now_loader:
                ...
                for batch_idx, (inputs, targets, indexes) in enumerate(loader):
                    ...
                    out_search, out_logits, out_softmax = net(inputs)
                    dist = torch.mm(out_search, out_memory)
...
class ProduceClass(nn.Module):
    ...
    def __init__(self, n_sample, low_dim, momentum=0.5):
        ...
        self.register_buffer('memory', torch.rand(self.n_sample, self.low_dim // 2))
        pass

    def update_label(self, out_search, out_softmax, indexes):
        batch_size = out_softmax.size(0)
        class_num = self.class_num.cpu()
        top_k = out_softmax.topk(self.low_dim, dim=1)[1].cpu()
        ...
        old_features = self.memory.index_select(0, indexes.data.view(-1)).resize_as_(out_search)
        old_features.mul_(self.momentum).add_(torch.mul(out_search.data, 1 - self.momentum))
        updated_weight = old_features
        self.memory.index_copy_(0, indexes, updated_weight)
        return class_labels

    def forward(self, out_labels, out, indexes, is_update=False, is_reset=False):
        if is_update:
            if is_reset:
                self.class_num.index_copy_(0, self.classes_index, torch.zeros(self.low_dim).long().cuda())
            classes = self.update_label(out_labels, out, indexes)
        else:
    ...
    def _train_one_epoch(self, epoch, max_epoch, update_epoch=3):
        ...
        if epoch % update_epoch == 0:
            ...
            for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
                ...
                out_search, out_logits, out_softmax = self.net(inputs)
                classes = self.produce_class(out_search, out_softmax, indexes,
                                             True, True if batch_idx == 0 else False)
                if batch_idx % 100 == 0:
        ...
        for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
            ...
            out_search, out_logits, out_softmax = self.net(inputs)
            targets = self.produce_class(out_search, out_softmax, indexes)
...
if __name__ == '__main__':
    ...
    """
    Top 1: 128: 69.55(momentum)|69.61(no momentum)|68.56(less)
          1024: 72.39
    """

    _low_dim = 1024
    _name = "5_class_{}_softmax_double".format(_low_dim)

    _momentum = 0.5
    _pre_train = None
    # _pre_train = "./checkpoint/5_class_128_softmax/ckpt.t7"
