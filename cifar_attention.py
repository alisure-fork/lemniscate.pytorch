...
class NearestAttention(nn.Module):
    ...
    def forward(self, pam_features, final_features, indexes):
        # k_nearest_indexes
        k_score, k_pam_features = self._get_k_nearest(pam_features.data, final_features.data, self.k_nearest)
        ...
        return k_score, k_pam_features, linear_average_out
    ...
    def _get_k_nearest(self, pam_features, final_features, k_nearest):
        ...
        return k_nearest_score, k_nearest_pam_features
...
class KNN(object):
    ...
    def knn(epoch, net, nearest_attention, train_loader, test_loader, k, t, recompute_memory=0):
        ...
        with torch.no_grad():
            ...
            now_loader = [test_loader, train_loader]
...
class AttentionLoss(nn.Module):
    ...
    def forward(self, pam_features, pam_features_2, linear_average_out, indexes):
        loss_1 = self.criterion(pam_features, pam_features_2)
...
class AttentionRunner(object):
    ...
    def _train_one_epoch(self, epoch, max_epoch):
        ...
        for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
            ...
            k_nearest_score, k_pam_features, linear_average_out = self.nearest_attention(
                pam_features, feature_final, indexes)
            pam_features_2 = self.pam_module(k_nearest_score, pam_features, k_pam_features)

            loss, loss_1, loss_2 = self.criterion(pam_features, pam_features_2, linear_average_out, indexes)
