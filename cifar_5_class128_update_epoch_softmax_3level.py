...
class AttentionResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_softmax2 = self.softmax(out_logits2)

        out_logits3 = self.linear_10(out_logits2)
        out_softmax3 = self.softmax(out_logits3)
        return out_logits, out_softmax, out_logits2, out_softmax2, out_logits3, out_softmax3
    ...
    def knn(epoch, net, produce_class, produce_class2, produce_class3,
        ...
        if recompute_memory:
            ...
            for batch_idx, (inputs, _, indexes) in enumerate(temp_loader):
                out_logits, out, out_logits2, out2, out_logits3, out3 = net(inputs)
                batch_size = inputs.size(0)
                out_memory[:, batch_idx * batch_size:batch_idx * batch_size + batch_size] = out.data.t()
                out_memory2[:, batch_idx * batch_size:batch_idx * batch_size + batch_size] = out2.data.t()
                out_memory3[:, batch_idx * batch_size:batch_idx * batch_size + batch_size] = out3.data.t()
                pass
        ...
        with torch.no_grad():
            ...
            for loader in now_loader:
                ...
                for batch_idx, (inputs, targets, indexes) in enumerate(loader):
                    ...
                    out_logits, out, out_logits2, out2, out_logits23, out3 = net(inputs)
                    dist = torch.mm(out, out_memory)
                    dist2 = torch.mm(out2, out_memory2)
                    dist3 = torch.mm(out3, out_memory3)
...
class AttentionRunner(object):
    ...
    def __init__(self, low_dim=512, low_dim2=128, low_dim3=10, momentum=0.5, learning_rate=0.03, resume=False,
    ...
    def _train_one_epoch(self, epoch, max_epoch, update_epoch=3):
        ...
        if epoch % update_epoch == 0:
            ...
            for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
                ...
                out_logits, out, out_logits2, out2, out_logits3, out3 = self.net(inputs)
                self.produce_class(out, indexes, True, True if batch_idx == 0 else False)
                self.produce_class2(out2, indexes, True, True if batch_idx == 0 else False)
                self.produce_class3(out3, indexes, True, True if batch_idx == 0 else False)
                pass
        ...
        for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
            ...
            targets2 = self.produce_class2(out2, indexes)
            targets3 = self.produce_class3(out3, indexes)
...
if __name__ == '__main__':
    ...
    _low_dim2 = 128
    _low_dim3 = 10
    _name = "5_class_{}_softmax_3level_{}_{}_1000".format(_low_dim, _low_dim2, _low_dim3)
