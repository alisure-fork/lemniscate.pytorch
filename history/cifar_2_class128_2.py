...
class AttentionResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, low_dim=128):
        ...
        self.linear_10 = nn.Linear(low_dim, 10)
        self.l2norm_10 = Normalize(2)
        pass
    ...
    def forward(self, x):
        ...
        out = self.l2norm_10(out)
        out_10 = out
        return out_128, out_10
...
class AttentionLoss(nn.Module):
    ...
    def forward(self, out_128, targets_128, out_10, targets_10):
        loss_1 = self.criterion_no(out_128, targets_128)
        loss_2 = self.criterion_no(out_10, targets_10)
...
class ProduceClass(nn.Module):
    ...
    def update_label(self, out, indexes):
        ...
        updated_weight = old_features.div(old_features.pow(2).sum(1, keepdim=True).pow(0.5))
        ...
        for i in range(batch_size):
            for j in top_k[i]:
                if self.class_per_num > self.class_num[j]:
                    ...
                    self.class_num.index_copy_(0, j, self.class_num[j] + 1)
                    break
                pass
            pass
        classes = torch.tensor(class_labels).long().cuda()

        # update
        self.classes.index_copy_(0, indexes, classes)
...
class AttentionRunner(object):
    ...
    def __init__(self, low_dim=128, momentum=0.5, learning_rate=0.005, resume=False,
        ...
        self.produce_class_10 = ProduceClass(n_sample=self.train_num, low_dim=10).cuda()
    ...
    def _train_one_epoch(self, epoch, max_epoch, update_epoch=3):
        ...
        if epoch % update_epoch == 0:
            ...
            for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
                ...
                out, out_10 = self.net(inputs)
                classes = self.produce_class(out, indexes, True, True if batch_idx == 0 else False)
                _ = self.produce_class(out_10, indexes, True, True if batch_idx == 0 else False)
                if batch_idx % 50 == 0:
        ...
        for batch_idx, (inputs, _, indexes) in enumerate(self.train_loader):
            ...
            out_128, out_10 = self.net(inputs)
            targets = self.produce_class(out_128, indexes)
            targets_10 = self.produce_class_10(out_10, indexes)

            loss, loss_1, loss_2 = self.criterion(out_128, targets, out_10, targets_10)
...
if __name__ == '__main__':
    ...
    runner = AttentionRunner(low_dim=_low_dim, resume=False, pre_train=pre_train,
                             checkpoint_path="./checkpoint/attention_class_{}/ckpt.t7".format(_low_dim))
